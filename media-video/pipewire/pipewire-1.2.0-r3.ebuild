...
pkg_postinst() {
	...
	if use jack-sdk; then
		ewarn
		ewarn "You have enabled the jack-sdk USE flag, which installs"
		ewarn "PipeWire implementation of JACK APIs (aka JACK3)."
		ewarn "While libjack.so is mostly working, libjackserver.so mostly does not."
		ewarn "If you actually use libjackserver.so (e.g. via media-sound/jackdbus),"
		ewarn "beware of suboptimal experience and/or things not (yet) working."
		ewarn "See https://gitlab.freedesktop.org/pipewire/pipewire/-/issues/819"
		ewarn
	fi
